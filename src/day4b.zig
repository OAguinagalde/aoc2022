...
fn are_ranges_fully_separate(range1: Range, range2: Range) bool {
    return (range1.start < range2.start and range1.end < range2.start) or (range1.start > range2.end and range1.end > range2.end);
}
...
pub fn run() !void {
    ...
    while (try reader.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        ...
        if (!are_ranges_fully_separate(input.range1, input.range2)) {
