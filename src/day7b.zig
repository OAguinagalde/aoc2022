...
pub fn run() !void {
    ...
    const used_space = fs.directories.items[try fs.get_root()].size;
    const current_free = total - used_space;
    ...
    std.debug.print("{d}{s} -> {s} ({d})\n", .{day, @tagName(part), fs.get_name(FileSystem.Directory, current_solution), fs.directories.items[current_solution].size});
}
